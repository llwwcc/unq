				...
				>
					<view class="center">
						<view class="name-tel">
							...
							<view class="tel">{{row.mobilePhone}}</view>
							<view class="default" v-if="row.isDefault">
								默认
							</view>
						</view>
						<view class="address">
							{{row.region}} {{row.detailAddress}}
						</view>
					</view>
					<view class="right">
						<view class="icon bianji" @tap.stop="edit(row)">
							<image :src="imgUrl+'/upload/static/list_Button_Edit.png'" 
	...
	export default {
		...
		methods:{
			getDetailsData() {
			    ...
			    uni.request({
					url:"https://wx.zzsnqyjxh.org/test/app/appAddress/query?userId=1",
					// data: {
					// // // 	// userId: _this.data.globalData.userInfo.id
			...
			edit(row){
				uni.setStorage({
					...
					success() {
						uni.navigateTo({
							url:"/pages/address/addressManage?type=edit"
						})
					}
				});
